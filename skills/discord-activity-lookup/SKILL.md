...
description: Query Discord activity — uses the discord_activity tool for fast queries, falls back to JSONL file reads.
...
Use this skill when the user asks about their past Discord activity — songs played, games played, online status, etc.
...
**File location:** `~/.hermes/profiles/<profile>/logs/discord-activity/YYYY-MM-DD.jsonl`

**Approach:**
1. Parse the time period from the user's query
2. For short periods (< 1 day): read the relevant daily file(s)
3. For longer periods: read multiple daily files
